...
export default function Mangas() {
    ...
    const checks = useSelector((state) => state.mangas.checks); // Obtener el estado de los checks desde Redux
    ...
    useEffect(() => {
        if (checks.length === 0) {
            ...
            const filtered = mangas.filter((manga) => {
                const mangaCategoryId = manga.category_id._id;
                return checks.includes(mangaCategoryId);
            });
    ...
    const handleCategoryChange = (categoryId) => {
        const updatedChecks = checks.includes(categoryId)
            ? checks.filter((id) => id !== categoryId)
            : [...checks, categoryId];

        setSelectedCategories(updatedChecks);
        dispatch(save_checks(updatedChecks)); // Actualiza los checks en el estado global
    };
