...
const { read_comments } = comments_actions
...
export default function chapters() {
  ...
  let total = useSelector(store => store.comments.total)
  ...
  useEffect(() => {}, [nextPage, prevPage, currentPage]);
  return (
    <>
          <div className="flex flex-col justify-between items-center h-screen bg-gray-100">
        <Nav_2/>
        <div className="relative flex justify-center items-center  w-full   h-screen   mb-10    bg-contain bg-center bg-no-repeat"
          ...
          <div className="flex  justify-between w-full  md:w-[90%] lg:w-[70%] xl:w-[52%] md:px-20 h-[90%] absolute">
            <div className=" justify-start items-center  flex  xl:hover:bg-[#0000000a] w-[48%]  h-auto lg:mr-12 mr-8  md:mt-0"
              onClick={prev}>
              <ArrowChapter d={d_left}/>
            </div>
            <div className=" flex justify-end items-center xl:hover:bg-[#0000000a]  w-[48%]   h-auto  md:mt-0   "
              onClick={next}>
          ...
          <div className="  flex self-end  w-[80%] justify-center items-center -mb-9 md:-mb-10  ">
            <img onClick={() => setShow(!show)} className="md:h-9 md:w-10 mt-12 md:mt-8 "
              src={icon_comment}
              alt="comments" />
            <div className="flex justify-center items-center  h-8 w-8 border-2 font-semibold border-slate-400 mr-4 ml-4 mt-12 md:mt-7 rounded">
              <h3>{total}</h3>
            </div>
          </div>
        </div>
      </div>
      {show && < Comments_ch show={show} setShow={setShow} />}
    </>
  );
}
